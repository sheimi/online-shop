...
class APIKeyAuthentication(Authentication):
    """
    Requires a model that has at least two fields, "key" and "secret"
    , which will be searched for when authing a request
    """
    ...
    def authorize(self):
        ...
        if 'key' in request.args and 'secret' in request.args:
            g.api_key = self.get_key(request.args['key'],
                    request.args['secret'])
...
class RestResource(object):
    ...
    def __init__(self, rest_api, model, authentication, allowed_methods=None):
        ...
        self._fields = {
            self.model: self.fields or self.model._meta.get_field_names()
        }
        if self.exclude:
        ...
        if self.include_resources:
            for field_name, resource in self.include_resources.items():
                ...
                resource_obj = resource(self.api, field_obj.to,
    ...
    def serialize_object(self, obj):
        ...
        return self.prepare_data(
            obj, s.serialize_object(obj, self._fields,
                self._exclude, self._include_foreign_keys)
        )

    def serialize_query(self, query):
        s = self.get_serializer()
        return [
            self.prepare_data(obj, s.serialize_object(obj, self._fields,\
                self._exclude, self._include_foreign_keys)) for obj in query
        ]
    ...
    def get_urls(self):
        return (
            ('/', self.require_method(self.api_list,
                ['GET', 'POST'])),
    ...
    def get_request_metadata(self, paginated_query):
        ...
        if current_page < paginated_query.get_pages():
            request_arguments[var] = current_page + 1
            next = url_for(self.get_url_name('api_list'),
                    **request_arguments)
...
class RestrictOwnerResource(RestResource):
    ...
    def save_object(self, instance, raw_data):
        ...
        return super(RestrictOwnerResource, self)\
...
class RestAPI(object):
    ...
    def register(self, model, provider=RestResource,
                 auth=None, allowed_methods=None):
        self._registry[model] = provider(self, model,
                                auth or self.default_auth, allowed_methods)
    ...
    def register_blueprint(self, **kwargs):
        self.app.register_blueprint(self.blueprint,
                url_prefix=self.url_prefix, **kwargs)
